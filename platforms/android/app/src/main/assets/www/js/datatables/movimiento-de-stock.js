...
$(document).ready(function () {
  ...
  if (
    ...
    $.ajax({
      url: "https://quattropy.com/valeria/s1/public/api/stock/datatables/movimiento-de-stock",
      method: "GET",
      dataType: "json",
      async: true,
      beforeSend: function () {
  ...
  function initializeDataTable(result) {
    var miTabla = $("#datatable-movimiento-de-stock").DataTable({
      data: result,
      columns: [
        { data: "movi_id", title: "ID" },
        { data: "operacion", title: "Operación" },
        { data: "dep1_nombre", title: "Depósito remitente" },
        { data: "prod_nombre", title: "Producto" },
        { data: "dep1_agente", title: "Remitente" },
        { data: "dep2_nombre", title: "Dep. receptor" },
        { data: "dep2_agente", title: "Recibido por" },
        { data: "idmaq", title: "Máq" },
        { data: "qr_code", title: "QR" },
        { data: "fechahora_mlog", title: "Fecha y Hora" },
      ],
      language: idioma_espanol,
      rowCallback: function (row, data) {
        console.log(data)
        if (data.operacion === "entrada") {
          $(row).addClass("entrada-row");
        } else if (data.operacion === "salida") {
